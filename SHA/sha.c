...
// Padding and preprocessing the message
...
void sha256_final(uint32_t *state, uint8_t *hash, uint8_t *buffer, size_t buffer_len, uint64_t total_len) {
    ...
    total_len <<= 3;
    buffer[56] = total_len >> 56;
    buffer[57] = total_len >> 48;
    buffer[58] = total_len >> 40;
    buffer[59] = total_len >> 32;
    buffer[60] = total_len >> 24;
    buffer[61] = total_len >> 16;
    buffer[62] = total_len >> 8;
    buffer[63] = total_len;
    sha256_transform(state, buffer);
...
void sha256(const uint8_t *data, size_t len, uint8_t *hash) {
    ...
    sha256_final(state, hash, buffer, 0, len);
}
...
/* Enable performance counters */
    ...
    const char *message = "RISC-V BOOM Core SHA Encryption Example";
    uint8_t hash[32];
