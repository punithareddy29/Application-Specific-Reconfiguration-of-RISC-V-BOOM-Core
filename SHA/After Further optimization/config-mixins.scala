          ...
          tileParams = BoomTileParams(
            core = BoomCoreParams(
              ...
              issueParams = Seq(
                ...
                IssueParams(issueWidth=2, numEntries=8, iqType=IQT_FP.litValue , dispatchWidth=2)),
              numIntPhysRegisters = 52,
              numFpPhysRegisters = 48,  //change
              numLdqEntries = 16,
              numStqEntries = 16,
              maxBrCount = 8,
            ...
            dcache = Some(
              DCacheParams(rowBits = site(SystemBusKey).beatBits, nSets=64, nWays=8, nMSHRs=2, nTLBWays=8)
            ),
            icache = Some(
...
class WithNCS152DefaultBooms(n: Int = 1, overrideIdOffset: Option[Int] = None) extends Config(
  ...
  new Config((site, here, up) => {
    case TilesLocated(InSubsystem) => {
      ...
      val idOffset = overrideIdOffset.getOrElse(prev.size)
      (0 until n).map { i =>
        val coreWidth = 4                     // CS152: Change me (1 to 4)
        val memWidth = 1                      // CS152: Change me (1 or 2)
        ...
        BoomTileAttachParams(
          tileParams = BoomTileParams(
            core = BoomCoreParams(
              ...
              numRasEntries = 32,               // CS152: Change me
...
class WithTAGELBPD extends Config((site, here, up) => {
  case TilesLocated(InSubsystem) => up(TilesLocated(InSubsystem), site) map {
    case tp: BoomTileAttachParams => tp.copy(tileParams = tp.tileParams.copy(core = tp.tileParams.core.copy(
      ...
      globalHistoryLength = 128,
      localHistoryLength = 16,
      localHistoryNSets = 0,
